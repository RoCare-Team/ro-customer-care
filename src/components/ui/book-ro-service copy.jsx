    ...
    setTimeout(() => {
      if (lead_type !== null) {
        fetch('https://waterpurifierservicecenter.in/customer/ro_customer/all_services.php', {
          ...
          body: JSON.stringify({ lead_type: lead_type })
        })
        .then(res => res.json())
        .then(data => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
            {services.map((service, index) => {
              ...
              return (
                <div
                  key={service.id}
                  className={`relative bg-white rounded-2xl md:rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden group transform ${
                    animateCard ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
                  }`}
                  style={{ 
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2 md:space-x-6">
                  ...
                  <div className="flex items-center space-x-2 md:space-x-3">
                    <div className={`relative w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-xs md:text-sm font-bold transition-all duration-300 ${
                      currentStep >= 1 
                  ...
                  <div className="relative w-8 md:w-16 h-1 bg-gray-200 rounded-full overflow-hidden">
                    <div className={`absolute inset-y-0 left-0 bg-gradient-to-r from-blue-600 to-blue-500 rounded-full transition-all duration-500 ${
                ...
                <div className="space-y-4 md:space-y-6">
                          ...
                          >
                            <div className="flex items-start space-x-2 md:space-x-3">
                              <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                                address.type === 'home' ? 'bg-green-100' : address.type === 'work' ? 'bg-blue-100' : 'bg-gray-100'
                              }`}>
                                <AddressIcon className={`w-4 md:w-5 h-4 md:h-5 ${
                                  address.type === 'home' ? 'text-green-600' : address.type === 'work' ? 'text-blue-600' : 'text-gray-600'
                                }`} />
                              </div>
                              <div className="flex-1">
                        ...
                        <div className="flex space-x-2 md:space-x-3">
                          {['home', 'work', 'other'].map((type) => (
                            <button
                              ...
                              className={`flex-1 py-1.5 md:py-2 px-3 md:px-4 rounded-lg md:rounded-xl font-medium transition-all duration-300 capitalize text-xs md:text-sm ${
                  ...
                  <div className="sticky bottom-0 bg-white pt-3 md:pt-4 border-t border-gray-100">
                    <button
                      ...
                      className={`w-full py-3 md:py-4 px-4 md:px-6 rounded-xl md:rounded-2xl transition-all duration-300 font-bold text-sm md:text-lg flex items-center justify-center space-x-2 md:space-x-3 transform hover:scale-105 shadow-lg ${
